      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        ...
        {mode === 'study' && (
          <div className="space-y-4">
            <p className="text-[#6b7280] text-sm italic">{chapter.summary}</p>
            <a href="/golf_textbook.pdf" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-sm text-[#00af51] hover:underline mb-2">
              📄 Golf Textbook (PDF)
            </a>
            {chapter.reading ? (
              <div className="space-y-4">
                {chapter.reading.split('\n\n').map((paragraph, i) => (
                  <p key={i} className="text-sm text-white leading-relaxed">{paragraph}</p>
                ))}
              </div>
            ) : (
