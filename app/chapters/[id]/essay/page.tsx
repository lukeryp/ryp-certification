  ...
  return (
    <div className="min-h-screen pb-20 md:pb-0">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        <Link href={`/chapters/${chapterId}`} className="text-gray-500 text-sm hover:text-gray-400 mb-4 block">
          &#8249; Back to Chapter
        </Link>

        {/* Header */}
        <div className="mb-6">
          <span className="text-xs text-gray-500">Chapter {chapterId} — Discussion Questions</span>
          <div className="flex items-center justify-between mt-1">
            <h1 className="text-xl font-bold text-gray-100">{currentQuestion.title}</h1>
            {isFullyCertified && (
              <span className="text-xs bg-green-900/50 text-green-400 px-3 py-1 rounded-full">Certified</span>
            )}
          </div>
        </div>

        {/* Progress summary */}
        <div className="flex items-center gap-4 mb-6 p-3 bg-[#111827] rounded-lg border border-gray-800">
          <div className="text-center">
            <div className="text-lg font-bold text-gray-100">{completedCount}/{questions.length}</div>
        ...
        <div className="bg-[#111827] rounded-xl border border-gray-800 p-5 mb-4">
          ...
          <p className="text-gray-200 text-sm leading-relaxed whitespace-pre-wrap">{currentQuestion.prompt}</p>
        </div>
          ...
          <div>
            <div className="relative">
              <textarea
                ...
                rows={10}
                className="w-full bg-[#111827] border border-gray-700 rounded-xl p-4 text-gray-200 text-sm leading-relaxed resize-none focus:outline-none focus:border-green-700 placeholder-gray-600"
              />
            </div>
            <div className="flex items-center justify-between mt-2 mb-4">
              <span className={`text-xs ${overLimit ? 'text-red-400 font-medium' : wordCount > currentQuestion.wordLimit * 0.85 ? 'text-yellow-400' : 'text-gray-500'}`}>
            ...
            <button
              ...
              disabled={!canSubmit}
              className="w-full py-3 bg-green-600 hover:bg-green-700 disabled:opacity-30 disabled:hover:bg-green-600 text-white font-medium rounded-lg transition-colors"
            >
              Submit for Grading
            </button>
            {existingAttempt && (
              <div className="mt-3 p-3 bg-[#111827] rounded-lg border border-gray-800 text-xs text-gray-400">
                <span className="font-medium text-gray-300">Previous best:</span>{' '}
        ...
        {questionState === 'grading' && (
          <div className="py-12 text-center">
            ...
            <p className="text-xs text-gray-600 mt-2">This usually takes 5–10 seconds</p>
          </div>
        )}
          ...
          <div>
            ...
            <div className="space-y-4 mb-6">
              <div className="p-4 bg-[#111827] rounded-xl border border-gray-800">
                <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Feedback</h3>
                <p className="text-sm text-gray-300 leading-relaxed whitespace-pre-wrap">{gradeResult.feedback}</p>
              </div>

              {gradeResult.suggestions && (
                <div className="p-4 bg-[#111827] rounded-xl border border-gray-800">
                  <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">To Go Deeper</h3>
                  <p className="text-sm text-gray-300 leading-relaxed whitespace-pre-wrap">{gradeResult.suggestions}</p>
                </div>
              )}
            </div>
            ...
            <div className="flex gap-3">
              <button
                onClick={handleRetry}
                className="flex-1 py-3 bg-[#111827] border border-gray-700 text-gray-300 font-medium rounded-lg hover:border-gray-600 transition-colors text-sm"
              >
                Revise Answer
              </button>
              {currentIndex < questions.length - 1 ? (
                <button
                  ...
                  className="flex-1 py-3 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition-colors text-sm"
                >
                  Next Question &#8250;
                </button>
              ) : (
                <Link href={`/chapters/${chapterId}`} className="flex-1">
                  <button className="w-full py-3 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition-colors text-sm">
                    Back to Chapter
                  </button>
                </Link>
              )}
